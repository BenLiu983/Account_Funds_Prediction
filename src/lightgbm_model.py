...
class lightgbm_dev_v2:
    ...
    def baseline_model(self):
        with mlflow.start_run(run_name = "Baseline Model") as run:
            ...
            mlflow.log_param("run_id", run_id)
            mlflow.lightgbm.log_model(model, "baseline model")
            print(f"MLflow Run ID: {run_id}")
    ...
    def hyperopt_model(self, max_evals=50):
        ...
        with mlflow.start_run(run_name = "Hyperopt Model") as run:
            ...
            run_id = run.info.run_id
            mlflow.log_param("run_id", run_id)
            mlflow.lightgbm.log_model(model, "hyperopt model")
...
class lightgbm_dev_v4:
    def __init__(self, X_train, y_train, X_test, y_test,
                 baseline_params=None, baseline_ind=0,
                 search_space=None, loss_function='rmse', max_evals=20):
    ...
    def hyperopt_model(self, max_evals=50):
        ...
        def objective(params):
            ...
            if self.loss_function == "rmse":
                loss = rmse_test
            elif self.loss_function == 'power_ratio':
                loss = -pr_test
